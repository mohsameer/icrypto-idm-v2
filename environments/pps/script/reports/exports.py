...
users_data = None
try:
	postgress_connection = psycopg2.connect(database="midpoint", user='midpoint', password='XXXXXXXXXXX', host='localhost', port= '5432')
...
for row in users_data:
	...
	row['HRFeedIdMatch'] = 'Unmatched'
	row['HRFeedNameMatch'] = 'Unmatched'
	if row['hrfeeddata'] and row['givenname'] and row['familyname']:
		row_hr_feed_data = json.loads(row['hrfeeddata'])
		if row_hr_feed_data.get('employeeIDNumber') == row['said']:
			row['HRFeedIdMatch'] = 'Matched'
		if row_hr_feed_data['employeeGivenName'].lower() == row['givenname'].lower() and row_hr_feed_data['employeeSurname'].lower() == row['familyname'].lower():
			row['HRFeedNameMatch'] = 'Matched'
...
for row in users_data:
	...
	if row['hrfeeddata'] and row['hremployeeisactive'] != '1':
		hr_feed_inactive.append(row)
...
generate_csv_from_array("HrFeedPresentButInactive", hr_feed_inactive)
